...
function Background3D(props) {
  ...
  useEffect(() => {
    ...
    const logos = new Array(logoCount);
    ...
    const logo_sphere = new LogoSpere(logos, 2.5, 3, 5, 0.5, new Vector3(0, 3, 0));
    scene.add(logo_sphere.object);
    ...
    function addSkyGradiant() {
      ...
      //set the colors of the sky gradiant black/purple for dark mode white/blue for light mode
      for (let i = 0; i < count; i++) {
        ...
        if (t <= 0) {
          if(light) {
            ...
            color.setRGB(0.1, 0.0, 0.2); //blueish purple
          }
